...
function loadContext(source) {
  // The generated block is stripped before matching. Keep the late-addition
  // hook callable while evaluating that stripped source.
  const ctx = { window: {}, console, applyPendingOfficialModelSyncUpdates() {} };
...
function buildBlock(updates, extensions, stableIdUpdates) {
  return `${START}
...
const localModelStableIdUpdates = ${JSON.stringify(stableIdUpdates, null, 2)};
...
const stableIdUpdates = localModels.map((model, index) => ({
  index,
  matchName: model.name || "",
  matchRealname: model.realname || "",
  matchBase: model.base || "",
  id: stableModelIds[index]
}));
...
const block = buildBlock(updates, extensions, stableIdUpdates);
...
console.log(JSON.stringify({
  ...
  stableIds: stableIdUpdates.length,
