...
На мобильных экранах до 480px верхняя панель `#builderMain > .top-bar` прокручивается вместе со списком: настройки, поиск и вкладки находятся в обычном потоке. Фиксированная шапка и расчет `--mobile-fixed-topbar-height` остаются только у каталога `#cardsSection`.
